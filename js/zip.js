 ...
 * ZIP writer.
 *
 * Written by hand for the same reason as the PDF: a zip holding already
 * compressed files needs no compression of its own. PNG and PDF are both
 * compressed formats, so deflating them again buys almost nothing, and
 * the "stored" method is just a container -- a header per file, the bytes
 * verbatim, and a directory at the end listing where each one starts.
 *
 * The one piece with any substance is CRC-32, which every entry carries
 * twice and which unzip checks before it will extract anything.
 * ------------------------------------------------------------------ */
...
const ZipWriter = (() => {
  ...
  /* MS-DOS packed date and time: seconds in two-second steps, and years
     counted from 1980. A format old enough to be older than the files it
     is usually asked to carry. */
  function dosStamp(d) {
